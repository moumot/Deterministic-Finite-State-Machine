...
int startGen(string);
vector<vector <string>> dfaGen(string);
vector<string> statesGen(string);
vector<int> finalGen(string);
vector<string> alphaGen(string);
...
int main()
{
	...
	start1 = startGen("M1.dfa.txt");
	start2 = startGen("M3.dfa.txt");
	final1 = finalGen("M1.dfa.txt");
	final2 = finalGen("M3.dfa.txt");
	dfa1 = dfaGen("M1.dfa.txt");
	dfa2 = dfaGen("M3.dfa.txt");
	states1 = statesGen("M1.dfa.txt");
	states2 = statesGen("M3.dfa.txt");
	alphabet1 = alphaGen("M1.dfa.txt");
	alphabet2 = alphaGen("M3.dfa.txt");
...
vector<vector <string>> dfaGen(string filename) {
	...
	while (getline(myfile, line))
	{
		...
		else if (line.substr(0, line.find(':')) == "input_alphabet")
		{
			...
			while (counter != 0)
			{
				...
				dfa[verify(alphabet, alphaTemp, alphabetnum)][verify(states, stateTemp, statesnum)] = line.substr(line.find('>') + 1, line.find(';') - line.find('>') - 1);
				line.erase(0, line.find(';') + 1);
				counter--;
			}
...
vector<int> finalGen(string filename)
{
	string *states;
	int statesnum, i;
	vector< int > final;
...
vector<string> statesGen(string filename) {
	int statesnum, i;
	vector< string > states;
...
vector<string> alphaGen(string filename) {
	int alphabetnum, i;
	vector< string > alphabet;
...
string check(vector<vector <string>> dfa1, vector<vector <string>> dfa2, vector<int> final1, vector<int> final2, int start1, int start2, vector<string> states1, vector<string> states2, vector<string> alphabet1, vector<string> alphabet2)
{
	...
	//visited.resize(dfa1.size(), vector<int>(dfa1[0].size(), 0));
	output = "";
	output = bfs(ndfa,visited,final,0,start,states,output,alphabet1);
	//output = bfs(dfa1, visited, final1, 0, start1, states1, output, alphabet1);
